...
from luigi1.parameter import ParameterException
...
class WithDefault(luigi1.Task):
    x = luigi1.Parameter(default='xyz')


class Foo(luigi1.Task):
    bar = luigi1.Parameter()
    p2 = luigi1.IntParameter()
    multi = luigi1.Parameter(is_list=True)
    not_a_param = "lol"


class Bar(luigi1.Task):
    multibool = luigi1.BoolParameter(is_list=True)

    def run(self):
        Bar._val = self.multibool


class Baz(luigi1.Task):
    bool = luigi1.BoolParameter()

    def run(self):
        Baz._val = self.bool


class ForgotParam(luigi1.Task):
...
class ForgotParamDep(luigi1.Task):
...
class HasGlobalParam(luigi1.Task):
    x = luigi1.Parameter()
    global_param = luigi1.IntParameter(is_global=True, default=123)  # global parameters need default values
    global_bool_param = luigi1.BoolParameter(is_global=True, default=False)
...
class HasGlobalParamDep(luigi1.Task):
    x = luigi1.Parameter()

    def requires(self):
        return HasGlobalParam(self.x)

_shared_global_param = luigi1.Parameter(is_global=True, default='123')


class SharedGlobalParamA(luigi1.Task):
    shared_global_param = _shared_global_param


class SharedGlobalParamB(luigi1.Task):
    shared_global_param = _shared_global_param


class BananaDep(luigi1.Task):
    x = luigi1.Parameter()
    y = luigi1.Parameter(default='def')
...
class Banana(luigi1.Task):
    x = luigi1.Parameter()
    y = luigi1.Parameter()
    style = luigi1.Parameter(default=None)
...
class MyConfig(luigi1.Config):
    mc_p = luigi1.IntParameter()
    mc_q = luigi1.IntParameter(default=73)


class MyConfigWithoutSection(luigi1.ConfigWithoutSection):
    mc_r = luigi1.IntParameter()
    mc_s = luigi1.IntParameter(default=99)
...
class ParameterTest(unittest.TestCase):
    ...
    def test_missing_param(self):
        def create_a():
            return A()
        self.assertRaises(luigi1.parameter.MissingParameterException, create_a)

    def test_unknown_param(self):
        def create_a():
            return A(p=5, q=4)
        self.assertRaises(luigi1.parameter.UnknownParameterException, create_a)
    ...
    def test_duplicated_param(self):
        def create_a():
            return A(5, p=7)
        self.assertRaises(luigi1.parameter.DuplicateParameterException, create_a)
    ...
    def test_multibool(self):
        luigi1.run(['--local-scheduler', '--no-lock', 'Bar', '--multibool', 'true', '--multibool', 'false'])
        self.assertEqual(Bar._val, (True, False))

    def test_multibool_empty(self):
        luigi1.run(['--local-scheduler', '--no-lock', 'Bar'])
        self.assertEqual(Bar._val, tuple())

    def test_bool_false(self):
        luigi1.run(['--local-scheduler', '--no-lock', 'Baz'])
        self.assertEqual(Baz._val, False)

    def test_bool_true(self):
        luigi1.run(['--local-scheduler', '--no-lock', 'Baz', '--bool'])
        self.assertEqual(Baz._val, True)

    def test_forgot_param(self):
        self.assertRaises(luigi1.parameter.MissingParameterException, luigi1.run, ['--local-scheduler', '--no-lock', 'ForgotParam'], )

    @email_patch
    def test_forgot_param_in_dep(self, emails):
        # A programmatic missing parameter will cause an error email to be sent
        luigi1.run(['--local-scheduler', '--no-lock', 'ForgotParamDep'])
        self.assertNotEquals(emails, [])

    def test_default_param_cmdline(self):
        luigi1.run(['--local-scheduler', '--no-lock', 'WithDefault'])
        self.assertEqual(WithDefault().x, 'xyz')
    ...
    def test_global_param_cmdline(self):
        luigi1.run(['--local-scheduler', '--no-lock', 'HasGlobalParam', '--x', 'xyz', '--global-param', '124'])
        h = HasGlobalParam(x='xyz')
        self.assertEqual(h.global_param, 124)
        self.assertEqual(h.global_bool_param, False)

    def test_global_param_cmdline_flipped(self):
        luigi1.run(['--local-scheduler', '--no-lock', '--global-param', '125', 'HasGlobalParam', '--x', 'xyz'])
        h = HasGlobalParam(x='xyz')
    ...
    def test_global_param_dep_cmdline(self):
        luigi1.run(['--local-scheduler', '--no-lock', 'HasGlobalParamDep', '--x', 'xyz', '--global-param', '124'])
        h = HasGlobalParam(x='xyz')
        self.assertEqual(h.global_param, 124)
        self.assertEqual(h.global_bool_param, False)

    def test_global_param_dep_cmdline_optparse(self):
        luigi1.run(['--local-scheduler', '--no-lock', '--task', 'HasGlobalParamDep', '--x', 'xyz', '--global-param', '124'], use_optparse=True)
        h = HasGlobalParam(x='xyz')
        self.assertEqual(h.global_param, 124)
        self.assertEqual(h.global_bool_param, False)

    def test_global_param_dep_cmdline_bool(self):
        luigi1.run(['--local-scheduler', '--no-lock', 'HasGlobalParamDep', '--x', 'xyz', '--global-bool-param'])
        h = HasGlobalParam(x='xyz')
        self.assertEqual(h.global_param, 123)
        self.assertEqual(h.global_bool_param, True)

    def test_global_param_shared(self):
        luigi1.run(['--local-scheduler', '--no-lock', 'SharedGlobalParamA', '--shared-global-param', 'abc'])
        b = SharedGlobalParamB()
        self.assertEqual(b.shared_global_param, 'abc')

    def test_insignificant_parameter(self):
        class InsignificantParameterTask(luigi1.Task):
            foo = luigi1.Parameter(significant=False, default='foo_default')
            bar = luigi1.Parameter()
    ...
    def test_local_significant_param(self):
        ...
        class MyTask(luigi1.Task):
            ...
            x = luigi1.Parameter(significant=True)

        MyTask('arg')
        self.assertRaises(luigi1.parameter.MissingParameterException,
                          lambda: MyTask())
    ...
    def test_local_insignificant_param(self):
        ...
        class MyTask(luigi1.Task):
            ...
            x = luigi1.Parameter(significant=False)
...
class TestNewStyleGlobalParameters(unittest.TestCase):
    ...
    def test_x_arg(self):
        luigi1.run(['--local-scheduler', '--no-lock', 'Banana', '--x', 'foo', '--y', 'bar', '--style', 'x-arg'])
        self.expect_keys(['banana-foo-bar', 'banana-dep-foo-def'])

    def test_x_arg_override(self):
        luigi1.run(['--local-scheduler', '--no-lock', 'Banana', '--x', 'foo', '--y', 'bar', '--style', 'x-arg', '--BananaDep-y', 'xyz'])
        self.expect_keys(['banana-foo-bar', 'banana-dep-foo-xyz'])

    def test_x_arg_override_stupid(self):
        luigi1.run(['--local-scheduler', '--no-lock', 'Banana', '--x', 'foo', '--y', 'bar', '--style', 'x-arg', '--BananaDep-x', 'blabla'])
        self.expect_keys(['banana-foo-bar', 'banana-dep-foo-def'])

    def test_x_arg_y_arg(self):
        luigi1.run(['--local-scheduler', '--no-lock', 'Banana', '--x', 'foo', '--y', 'bar', '--style', 'x-arg-y-arg'])
        self.expect_keys(['banana-foo-bar', 'banana-dep-foo-bar'])

    def test_x_arg_y_arg_override(self):
        luigi1.run(['--local-scheduler', '--no-lock', 'Banana', '--x', 'foo', '--y', 'bar', '--style', 'x-arg-y-arg', '--BananaDep-y', 'xyz'])
        self.expect_keys(['banana-foo-bar', 'banana-dep-foo-bar'])

    def test_x_arg_y_arg_override_all(self):
        luigi1.run(['--local-scheduler', '--no-lock', 'Banana', '--x', 'foo', '--y', 'bar', '--style', 'x-arg-y-arg', '--BananaDep-y', 'xyz', '--BananaDep-x', 'blabla'])
        self.expect_keys(['banana-foo-bar', 'banana-dep-foo-bar'])

    def test_y_arg_override(self):
        luigi1.run(['--local-scheduler', '--no-lock', 'Banana', '--x', 'foo', '--y', 'bar', '--style', 'y-kwarg', '--BananaDep-x', 'xyz'])
        self.expect_keys(['banana-foo-bar', 'banana-dep-xyz-bar'])

    def test_y_arg_override_both(self):
        luigi1.run(['--local-scheduler', '--no-lock', 'Banana', '--x', 'foo', '--y', 'bar', '--style', 'y-kwarg', '--BananaDep-x', 'xyz', '--BananaDep-y', 'blah'])
        self.expect_keys(['banana-foo-bar', 'banana-dep-xyz-bar'])

    def test_y_arg_override_banana(self):
        luigi1.run(['--local-scheduler', '--no-lock', 'Banana', '--y', 'bar', '--style', 'y-kwarg', '--BananaDep-x', 'xyz', '--Banana-x', 'baz'])
...
class TestRemoveGlobalParameters(unittest.TestCase):
    ...
    def run_and_check(self, args):
        run_exit_status = luigi1.run(['--local-scheduler', '--no-lock'] + args)
    ...
    def test_misc_1(self):
        class Dogs(luigi1.Config):
            n_dogs = luigi1.IntParameter()

        class CatsWithoutSection(luigi1.ConfigWithoutSection):
            n_cats = luigi1.IntParameter()
    ...
    def test_global_significant_param(self):
        ...
        class MyTask(luigi1.Task):
            ...
            x_g1 = luigi1.Parameter(default='y', is_global=True, significant=True)

        self.assertRaises(luigi1.parameter.UnknownParameterException,
                          lambda: MyTask('arg'))
    ...
    def test_global_insignificant_param(self):
        ...
        class MyTask(luigi1.Task):
            ...
            x_g2 = luigi1.Parameter(default='y', is_global=True, significant=False)
    ...
    def test_mixed_params(self):
        ...
        class MyTask(luigi1.Task):
            # This could typically be "--num-threads=True"
            x_g3 = luigi1.Parameter(default='y', is_global=True)
            local_param = luigi1.Parameter()

        MyTask('setting_local_param')

    def test_mixed_params_inheritence(self):
        """ A slightly more real-world like test case """
        class TaskWithOneGlobalParam(luigi1.Task):
            non_positional_param = luigi1.Parameter(default='y', is_global=True)

        class TaskWithOnePositionalParam(TaskWithOneGlobalParam):
            """ Try to mess with positional parameters by subclassing """
            only_positional_param = luigi1.Parameter()

            def complete(self):
                return True

        class PositionalParamsRequirer(luigi1.Task):
...
class TestParamWithDefaultFromConfig(unittest.TestCase):
    ...
    def testNoSection(self):
        self.assertRaises(ParameterException, lambda: luigi1.Parameter(config_path=dict(section="foo", name="bar")).value)
    ...
    def testDefault(self):
        class A(luigi1.Task):
            p = luigi1.Parameter(config_path=dict(section="foo", name="bar"))
    ...
    def testDateHour(self):
        p = luigi1.DateHourParameter(config_path=dict(section="foo", name="bar"))
        self.assertEqual(datetime.datetime(2001, 2, 3, 4, 0, 0), p.value)

    @with_config({"foo": {"bar": "2001-02-03"}})
    def testDate(self):
        p = luigi1.DateParameter(config_path=dict(section="foo", name="bar"))
        self.assertEqual(datetime.date(2001, 2, 3), p.value)

    @with_config({"foo": {"bar": "123"}})
    def testInt(self):
        p = luigi1.IntParameter(config_path=dict(section="foo", name="bar"))
        self.assertEqual(123, p.value)

    @with_config({"foo": {"bar": "true"}})
    def testBool(self):
        p = luigi1.BoolParameter(config_path=dict(section="foo", name="bar"))
        self.assertEqual(True, p.value)

    @with_config({"foo": {"bar": "2001-02-03-2001-02-28"}})
    def testDateInterval(self):
        p = luigi1.DateIntervalParameter(config_path=dict(section="foo", name="bar"))
        expected = luigi1.date_interval.Custom.parse("2001-02-03-2001-02-28")
        self.assertEqual(expected, p.value)

    @with_config({"foo": {"bar": "1 day"}})
    def testTimeDelta(self):
        p = luigi1.TimeDeltaParameter(config_path=dict(section="foo", name="bar"))
    ...
    def testTimeDelta8601YearMonthNotSupported(self):
        def f():
            return luigi1.TimeDeltaParameter(config_path=dict(section="foo", name="bar")).value
        self.assertRaises(luigi1.parameter.ParameterException, f)  # ISO 8601 durations with years or months are not supported
    ...
    def testTimeDelta8601MBeforeT(self):
        def f():
            return luigi1.TimeDeltaParameter(config_path=dict(section="foo", name="bar")).value
        self.assertRaises(luigi1.parameter.ParameterException, f)  # ISO 8601 durations with months are not supported

    def testHasDefaultNoSection(self):
        luigi1.Parameter(config_path=dict(section="foo", name="bar")).has_value
        self.assertFalse(luigi1.Parameter(config_path=dict(section="foo", name="bar")).has_value)
    ...
    def testHasDefaultWithBoth(self):
        self.assertTrue(luigi1.Parameter(config_path=dict(section="foo", name="bar")).has_value)

    @with_config({"foo": {"bar": "one\n\ttwo\n\tthree\n"}})
    def testDefaultList(self):
        p = luigi1.Parameter(is_list=True, config_path=dict(section="foo", name="bar"))
        self.assertEqual(('one', 'two', 'three'), p.value)

    @with_config({"foo": {"bar": "1\n2\n3"}})
    def testDefaultIntList(self):
        p = luigi1.IntParameter(is_list=True, config_path=dict(section="foo", name="bar"))
        self.assertEqual((1, 2, 3), p.value)

    @with_config({"foo": {"bar": "baz"}})
    def testWithDefault(self):
        p = luigi1.Parameter(config_path=dict(section="foo", name="bar"), default='blah')
    ...
    @with_config({"foo": {"bar": "baz"}})
    def testGlobal(self):
        p = luigi1.Parameter(config_path=dict(section="foo", name="bar"), is_global=True, default='blah')
    ...
    def testDefaultFromTaskNameWithDefault(self):
        class A(luigi1.Task):
            p = luigi1.Parameter(default="banana")
