...
class AmbiguousClass(luigi1.Task):
    pass
...
class NonAmbiguousClass(luigi1.ExternalTask):
    pass


class NonAmbiguousClass(luigi1.Task):

    def run(self):
        NonAmbiguousClass.has_run = True


class TaskWithSameName(luigi1.Task):
...
class WriteToFile(luigi1.Task):
    filename = luigi1.Parameter()

    def output(self):
        return luigi1.LocalTarget(self.filename)
...
class FooBaseClass(luigi1.Task):
    x = luigi1.Parameter(default='foo_base_default')
...
class CmdlineTest(unittest.TestCase):
    ...
    def test_setup_interface_logging(self, handler, logger):
        ...
        with mock.patch("luigi.interface.setup_interface_logging.has_run", new=False):
            luigi1.interface.setup_interface_logging()
        ...
        with mock.patch("luigi.interface.setup_interface_logging.has_run", new=False):
            if six.PY2:
                ...
                error = KeyError
            self.assertRaises(error, luigi1.interface.setup_interface_logging, '/blah')
    ...
    def test_cmdline_logger(self, setup_mock, warn):
        with mock.patch("luigi.interface.core") as env_params:
            ...
            luigi1.run(['SomeTask', '--n', '7', '--local-scheduler', '--no-lock'])
        ...
        with mock.patch("luigi.configuration.get_config") as getconf:
            ...
            luigi1.interface.setup_interface_logging.call_args_list = []
            luigi1.run(['SomeTask', '--n', '42', '--local-scheduler', '--no-lock'])
    ...
    def test_non_existent_class(self, print_usage):
        self.assertRaises(SystemExit, luigi1.run, ['--local-scheduler', '--no-lock', 'XYZ'])

    @mock.patch('argparse.ArgumentParser.print_usage')
    def test_no_task(self, print_usage):
        self.assertRaises(SystemExit, luigi1.run, ['--local-scheduler', '--no-lock'])
...
class InvokeOverCmdlineTest(unittest.TestCase):
    ...
    def test_bin_luigi(self):
        t = luigi1.LocalTarget(is_tmp=True)
...
class NewStyleParameters822Test(unittest.TestCase):
    ...
    def test_subclasses(self):
        ap = luigi1.interface.ArgParseInterface()
