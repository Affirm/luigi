...
class PostgresTarget(luigi1.Target):
    """
    Target for a resource in Postgres.

    This will rarely have to be directly instantiated by the user.
    """
    marker_table = luigi1.configuration.get_config().get('postgres', 'marker-table', 'table_updates')
...
# everything below will rarely have to be overridden
    ...
    def run(self):
        ...
        tmp_dir = luigi1.configuration.get_config().get('postgres', 'local-tmp-dir', None)
