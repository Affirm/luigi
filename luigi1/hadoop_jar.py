...
def fix_paths(job):
    ...
    args = []
    for x in job.args():
        if isinstance(x, luigi1.hdfs.HdfsTarget):  # input/output
            if x.exists() or not job.atomic_output():  # input
                args.append(x.path)
            else:  # output
                x_path_no_slash = x.path[:-1] if x.path[-1] == '/' else x.path
                y = luigi1.hdfs.HdfsTarget(x_path_no_slash + '-luigi-tmp-%09d' % random.randrange(0, 1e10))
...
class HadoopJarJobRunner(luigi1.hadoop.JobRunner):
    """
    ...
    def run_job(self, job):
        ...
        arglist = luigi1.hdfs.load_hadoop_cmd() + ['jar', job.jar()]
        if job.main():
        ...
        luigi1.hadoop.run_and_track_hadoop_job(arglist)

        for a, b in tmp_files:
            a.move(b)


class HadoopJarJobTask(luigi1.hadoop.BaseHadoopJobTask):
    """
