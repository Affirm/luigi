...
def run_and_track_hadoop_job(arglist, tracking_url_callback=None, env=None):
    ...
    def write_luigi_history(arglist, history):
        ...
        if history_filename and '-output' in arglist:
            ...
            f = luigi1.hdfs.HdfsTarget(os.path.join(output_dir, history_filename)).open('w')
...
class HadoopJobRunner(JobRunner):
    ...
    def run_job(self, job):
        packages = [luigi1] + self.modules + job.extra_modules() + list(_attached_packages)
        ...
        if self.end_job_with_atomic_move_dir:
            if isinstance(job.output(), luigi1.s3.S3FlagTarget):
        ...
        arglist = luigi1.hdfs.load_hadoop_cmd() + ['jar', self.streaming_jar]

        # 'libjars' is a generic option, so place it first
        libjars = [libjar for libjar in self.libjars]

        for libjar in self.libjars_in_hdfs:
            subprocess.call(luigi1.hdfs.load_hadoop_cmd() + ['fs', '-get', libjar, self.tmp_dir])
        ...
        for target in luigi1.task.flatten(job.input_hadoop()):
            if not isinstance(target, luigi1.hdfs.HdfsTarget) \
                    and not isinstance(target, luigi1.s3.S3Target):
                raise TypeError('target must be an HdfsTarget or S3Target')
            arglist += ['-input', target.path]

        if not isinstance(job.output(), luigi1.hdfs.HdfsTarget) \
                and not isinstance(job.output(), luigi1.s3.S3FlagTarget):
        ...
        if self.end_job_with_atomic_move_dir:
            luigi1.hdfs.HdfsTarget(output_hadoop).move_dir(output_final)
        self.finish()
...
class LocalJobRunner(JobRunner):
    ...
    def run_job(self, job):
        ...
        for i in luigi1.task.flatten(job.input_hadoop()):
...
class BaseHadoopJobTask(luigi1.Task):
    pool = luigi1.Parameter(is_global=True, default=None, significant=False)
    ...
    def input_local(self):
        return luigi1.task.getpaths(self.requires_local())

    def input_hadoop(self):
        return luigi1.task.getpaths(self.requires_hadoop())

    def deps(self):
        # Overrides the default implementation
        return luigi1.task.flatten(self.requires_hadoop()) + luigi1.task.flatten(self.requires_local())
...
class JobTask(BaseHadoopJobTask):
    ...
    def job_runner(self):
        ...
        """
        outputs = luigi1.task.flatten(self.output())
        for output in outputs:
            if not isinstance(output, luigi1.hdfs.HdfsTarget):
