...
from luigi1.target import FileAlreadyExists, FileSystemTarget
...
def load_hive_cmd():
    return luigi1.configuration.get_config().get('hive', 'command', 'hive')


def get_hive_syntax():
    return luigi1.configuration.get_config().get('hive', 'release', 'cdh4')
...
class HiveQueryTask(luigi1.hadoop.BaseHadoopJobTask):
    """
    ...
    def hiverc(self):
        ...
        """
        return luigi1.configuration.get_config().get('hive', 'hiverc-location', default=None)
    ...
    def hiveconfs(self):
        ...
        * hive.exec.reducers.max (reducers_max)
        ...
        if self.pool is not None:
            ...
            scheduler_type = luigi1.configuration.get_config().get('hadoop', 'scheduler', 'fair')
...
class HiveQueryRunner(luigi1.hadoop.JobRunner):
    """
    ...
    def run_job(self, job):
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            logger.info(arglist)
            return luigi1.hadoop.run_and_track_hadoop_job(arglist)


class HiveTableTarget(luigi1.Target):
    """
...
class HivePartitionTarget(luigi1.Target):
    """
...
class ExternalHiveTask(luigi1.ExternalTask):
    """
    External task that depends on a Hive table/partition.
    """

    database = luigi1.Parameter(default='default')
    table = luigi1.Parameter()
    ...
    partition = luigi1.Parameter(default=None, description='Python dictionary specifying the target partition e.g. {"date": "2013-01-25"}')
