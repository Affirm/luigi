...
class RemoteFileSystem(luigi1.target.FileSystem):
...
class AtomicRemoteFileWriter(luigi1.format.OutputPipeProcessWrapper):
...
class RemoteTarget(luigi1.target.FileSystemTarget):
    """
    ...
    def __init__(self, path, host, format=None, username=None, key_file=None):
        ...
        if format is None:
            format = luigi1.format.get_default_format()
        self.format = format
    ...
    def open(self, mode='r'):
        if mode == 'w':
            ...
            file_reader = luigi1.format.InputPipeProcessWrapper(
