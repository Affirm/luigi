...
import luigi1.contrib.target
import luigi1.format
from luigi1.target import FileAlreadyExists, FileSystem, FileSystemTarget

logger = logging.getLogger('luigi-interface')


class hdfs(luigi1.Config):
    client_version = luigi1.IntParameter(default=None)
    effective_user = luigi1.Parameter(default=None)
    snakebite_autoconfig = luigi1.BoolParameter(default=False)
    namenode_host = luigi1.Parameter(default=None)
    namenode_port = luigi1.IntParameter(default=None)
    client = luigi1.Parameter(default=None)
    use_snakebite = luigi1.BoolParameter(default=False)
    tmp_dir = luigi1.Parameter(config_path=dict(section='core', name='hdfs-tmp-dir'), default=None)
...
def load_hadoop_cmd():
    return luigi1.configuration.get_config().get('hadoop', 'command', 'hadoop').split()
...
class SnakebiteHdfsClient(HdfsClient):
    ...
    def mkdir(self, path, parents=True, mode=0o755, raise_if_exists=False):
        ...
        if raise_if_exists and "ile exists" in result[0].get('error', ''):
            raise luigi1.target.FileAlreadyExists("%s exists" % (path,))
        return result
...
def get_configured_hadoop_version():
    ...
    """
    return luigi1.configuration.get_config().get("hadoop", "version", "cdh4").lower()
...
def get_autoconfig_client(show_warnings=True):
    ...
    if configured_client == "snakebite_with_hadoopcli_fallback":
        return luigi1.contrib.target.CascadingClient([SnakebiteHdfsClient(),
...
class HdfsReadPipe(luigi1.format.InputPipeProcessWrapper):

    def __init__(self, path):
        super(HdfsReadPipe, self).__init__(load_hadoop_cmd() + ['fs', '-cat', path])


class HdfsAtomicWritePipe(luigi1.format.OutputPipeProcessWrapper):
    """
...
class HdfsAtomicWriteDirPipe(luigi1.format.OutputPipeProcessWrapper):
    """
...
class PlainFormat(luigi1.format.Format):
...
class PlainDirFormat(luigi1.format.Format):
...
class CompatibleHdfsFormat(luigi1.format.Format):
...
class HdfsTarget(FileSystemTarget):
    ...
    def __init__(self, path=None, format=None, is_tmp=False, fs=None):
        ...
        if format is None:
            format = luigi1.format.get_default_format() >> Plain
    ...
    @luigi1.util.deprecate_kwarg('fail_if_exists', 'raise_if_exists', False)
