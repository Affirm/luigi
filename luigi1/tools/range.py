...
from luigi1.target import FileSystemTarget
from luigi1.task import Register, flatten_output

logger = logging.getLogger('luigi-interface')


class RangeEvent(luigi1.Event):  # Not sure if subclassing currently serves a purpose. Stringly typed, events are.
    """
...
class RangeBase(luigi1.WrapperTask):
    """
    ...
    start = luigi1.Parameter()
    stop = luigi1.Parameter()
    reverse = luigi1.BoolParameter(
        default=False,
    ...
    task_limit = luigi1.IntParameter(
        default=50,
...
class RangeDailyBase(RangeBase):
    ...
    days_forward = luigi1.IntParameter(
        default=0,
...
class RangeHourlyBase(RangeBase):
    ...
    hours_forward = luigi1.IntParameter(
        default=0,
    ...
    def _format_datetime(self, dt):
        return luigi1.DateHourParameter().serialize(dt)
